...
async def process_user_id(message: Message, state: FSMContext):
    ...
    if user_id.isdigit():
        ...
        await process_not_user_id(message, state)


@super_admin_router.message(StateFilter(FSMAdminPanel.waiting_for_user_id))
async def process_not_user_id(message: Message, state: FSMContext):
    '''
    Хэндлер принимает не правельный ввод ID пользователя.
    '''
...
async def process_admin_id(message: Message, state: FSMContext):
    ...
    if user_id.isdigit():
        ...
        await process_not_admin_id(message, state)


@super_admin_router.message(StateFilter(FSMAdminPanel.waiting_for_admin_id))
async def process_not_admin_id(message: Message, state: FSMContext):
    '''
    Хэндлер принимает не правильный ввод ID админестратора.
    '''
