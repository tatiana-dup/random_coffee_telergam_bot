...
- обычный админ: может управлять ботом и участниками: изменять интервал формирования пар, просматривать список участников, ставить их на паузу или забирать доступ к боту, выгружать данные об участниках, встречах и фитбэке в Гугл Таблицу, отправлять рассылку участникам и др.
- супер-админ (ID супер-админов прописаны в файле .env): может делать все то же самое, что и обычный админ + выдавать/забирать роль обычного админа для пользователя (пользователь должен быть зарегистрирован в боте, т.е. его данные должны содержаться в базе данных), а также просматривать список обычных админов.
...
Дальнейшие изменения — мой вклад.
...
6. Узнайте ID всех супер-админов и ID телеграм-группы, например, с помощью этого бота @username_to_id_test_bot:
- когда запустите бота, он пришлет вам ваш ID;
- чтобы узнать ID другого пользователя, выберите в меню User и в открывшемся списке выберите нужного пользователя, чей ID хотите узнать;
- чтобы узнать ID группы, выберите в меню Chat и в открывшемся списке выберите нужную группу, ID которой хотите узнать;
7. Передать разработчику следующие данные:
- токен бота (получить у BotFather)
- ID телеграм-группы
- телеграм ID супер-админа/ов (только супер-админ может давать другим пользователям роль админа)
